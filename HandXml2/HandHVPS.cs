    ...
    public static class HandHVPS
    {
        public static void HandFile(System.Windows.Forms.RichTextBox textbox, string excelfile, List<string> txtfiles, List<string> sheets, bool ignoreflag)
        {
            ...
            try
            {
                ...
                Dictionary<string, string> sheetSqlFiles = new Dictionary<string, string>();
                foreach (var sheet in sheets)
                {
                    //读取Excel到内存中
                    int titleRowIndex = 0;
                    DataTable table = CommonHelper.ReadExcel(excelfile, sheet, ref titleRowIndex);
                    string sqlfilename = GetSqlFile(txtfiles, sheet);
                    if (null != table && !string.IsNullOrEmpty(sqlfilename))
                    {
                        ...
                        sheetSqlFiles.Add(sheet, sqlfilename);
                ...
                {
                    Dictionary<string, string[]> dbresult = ReadResult(sheetSqlFiles[resultTable.TableName]);
        ...
        private static string GetSqlFile(List<string> files, string sheetName)
        {
